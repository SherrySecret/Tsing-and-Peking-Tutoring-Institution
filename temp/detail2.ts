...
interface Teacher {  id: string;  name: string;  avatar: string;  role: string;  subject: string;  classes: string;  phone?: string;  email?: string;  introduction?: string;  courses: Course[];}
...
Page({
  ...
  // 联系教师  contactTeacher() {    const { phone } = this.data.teacher;        wx.showActionSheet({      itemList: ['拨打电话', '发送短信'],      success: (res) => {        if (res.tapIndex === 0) {          // 拨打电话          if (phone) {            wx.makePhoneCall({              phoneNumber: phone,              fail: () => {                wx.showToast({                  title: '拨号失败',                  icon: 'none'                });              }            });          } else {            wx.showToast({              title: '无可用电话号码',              icon: 'none'            });          }        } else if (res.tapIndex === 1) {          // 发送短信          wx.showToast({            title: '短信功能暂未开放',            icon: 'none'          });        }      }    });
  },
